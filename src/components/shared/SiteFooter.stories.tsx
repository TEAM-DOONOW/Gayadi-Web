...
export const PolicyDialog: Story = {
  play: async ({ canvas }) => {
    ...
    let dialog = await canvas.findByRole('dialog', { name: '개인정보 처리방침' })
    ...
    dialog = await canvas.findByRole('dialog', { name: '서비스 이용약관' })
    await expect(dialog).toHaveAccessibleName('서비스 이용약관')
    // Reproduce a previous close event arriving after the next notice opens.
